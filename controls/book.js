...
var Book = function() {}
Book.prototype.getRecommandedBooks = function(skip, limit, callback) {
	BookModel.find({
		isRecommanded: true
	}, '', {
		skip: skip ? skip : 0,
		limit: limit ? limit : 10,
		sort: 'recommandedDate'
	}).select('ename name images images.filename images.url isRecommanded')
		.populate({
			path: 'images',
			select: 'url filename',
			match: {
...
Book.prototype.getNewBooks = function(skip, limit, callback) {
	BookModel.count({
		isNewBook: true
		...
		if (error)
			...
			BookModel.find({
				...
				limit: limit ? limit : 10,
				sort: 'expressDate'
			}).select('ename name images images.filename images.url isNewBook')
				.populate({
					path: 'images',
					select: 'url filename',
					match: {
					...
					if (err)
						...
						callback(err, count, books);
				});
		}
	});
}
// 获取热门图书到首页
Book.prototype.getHotBooks = function(skip, limit, callback) {
	BookModel.find({
		isHot: true
	}, '', {
		skip: skip ? skip : 0,
		limit: limit ? limit : 10,
		sort: 'hotDate'
	}).select('ename name images images.filename images.url isHot')
		.populate({
			path: 'images',
			select: 'url filename',
			match: {
...
// 获取一个书籍的详细信息
Book.prototype.getOneBookCompleteInfo = function(bookid, callback) {
	BookModel.findById(bookid).populate({
		path: 'images',
		select: 'url filename',
		match: {
			isCurrentUsed: true,
		...
		path: 'authors',
		select: 'name secName introduction memo',
		match: {},
		options: {}
	}).populate({
		path: 'bookCategray',
...
// 设置星级评定
Book.prototype.setIncStar = function(bookid, userid, star, oldStar, callback) {
	BookModel.findById(bookid).select('appraisePersons').exec(function(err, book) {
		if (err)
			return callback(err);
		else {
			var up = {
			...
			if (star != oldStar) {
				...
				up = {};
			var flag = false;
			var appraisePersons = book.appraisePersons;
			for (var i = 0; i < appraisePersons.length; i++) {
				if (userid == appraisePersons[i])
					flag = true;
			}
			// 传入的用户还没有对本书籍进行评价
			if (flag == false) {
				up.$push = {
					appraisePersons: userid
				}
			}
			BookModel.update({
				_id: bookid
...
// 获取用户的读书信息
Book.prototype.getBookAppraise = function(bookid, callback) {
	BookModel.findById(bookid).select('appraisePersons appraisePersons.name appraisePersons.signature appraisePersons.httpAvatar appraisePersons.bookStar').populate({
		path: 'appraisePersons',
		select: 'name bookStar signature httpAvatar',
		match: {},
		options: {}
	}).exec(function(err, book) {
		if (err)
...
// 书籍搜索
Book.prototype.searchBooks = function(searchName, page, searchPageSize, callback) {
	var t_count = 0;
	var searchLike = new RegExp(searchName, 'ig');
	BookModel.count({
		$or: [{
			'name': searchLike
		}, {
			'authors.name': searchLike
		}]
	}, function(err, count) {
		if (err)
			return callback(err); {
			...
			}).select('name ename publicDate star price isStore discount secDiscount pageNumber banding authors authors.name images images.url images.filename bookCategray bookCategray.name publicCompany publicCompany.name')
				.skip(skip).limit(searchPageSize)
				.sort({
					publicDate: 'asc'
				})
				.populate({
					...
					match: {},
					options: {}
				}).exec(function(err_, books_) {
					if (err)
						return callback(err);
					return callback(err, t_count, books_);
				});
		}
	})
}
